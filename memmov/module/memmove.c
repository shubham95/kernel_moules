...
unsigned long source_addr,tgt_addr,nr_pages;
static pte_t *gpte;
                           ...
                           loff_t * offset)
{
        ...
        source_addr = *((unsigned long *)buf);
        tgt_addr    = *((unsigned long *)buf+8);
        nr_pages    = *((unsigned long *)buf+16);

        printk(KERN_INFO "Process pid [%d] source [%ld], target [%ld], nr_pages [%ld]\n", current->pid, source_addr,tgt_addr,nr_pages);
